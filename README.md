...
1- mvn clean package
2- cd deploy/docker
3- docker build -t event/backend:v5 .
3.1- docker tag event/backend:v5 event/backend:latest
4- cd ../deploy/kubernates/
...
5- kubectl create -f backend.yaml ## call backend service kubernates
5-1- kubectl create -f deploy-nginx.yaml ## call nginx loadbalancer
